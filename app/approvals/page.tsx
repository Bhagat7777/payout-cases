...
import { CheckCircle, Star, Search, Building2, TrendingUp, ArrowLeft, Plus, Eye, ExternalLink } from "lucide-react"
...
export default function ApprovalsPage() {
  ...
  useEffect(() => {
    const loadCases = async () => {
      try {
        setIsLoading(true)
        const data = await getCasesByType(supabase, "approval")
        setCases(data)
        setError(null)
      } catch (err) {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === "INSERT") {
            ...
            loadCases() // Reload to get complete data with firm info
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-400">Real-time feed of successful payouts</p>
              </div>
            </div>

            <Link href="/approvals/submit">
              <Button className="bg-gradient-to-r from-[#22C55E] to-[#00D1B2] hover:opacity-90 text-white">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
            <TabsList className="bg-white/5 border border-white/10">
              <TabsTrigger
                value="all"
                className="data-[state=active]:bg-[#22C55E]/20 data-[state=active]:text-[#22C55E]"
              >
                All
              </TabsTrigger>
              <TabsTrigger
          ...
          <div className="mb-4 flex items-center justify-between">
            <p className="text-gray-400">
              {filteredCases.length} approval{filteredCases.length !== 1 ? "s" : ""} found
            </p>
              ...
              <AnimatePresence>
                {filteredCases.map((caseItem, index) => (
                  <motion.div
                    ...
                    className="group"
                  >
                    <Card className="glass border-white/10 hover:border-[#22C55E]/30 transition-all duration-300 h-full">
                      <CardHeader>
                              ...
                              <div className="flex items-center space-x-1 mb-1">
                                {[...Array(5)].map((_, i) => (
                                  <Star
                                    key={i}
                                    className={`w-3 h-3 ${
                                      i < caseItem.rating! ? "text-yellow-400 fill-current" : "text-gray-600"
                                    }`}
                                  />
                                ))}
                              </div>
                      ...
                      <CardContent>
                        <h3 className="text-[#E6E7EB] font-semibold mb-2">{caseItem.title || "Payout Approved"}</h3>
                        <p className="text-gray-300 text-sm mb-4 line-clamp-3">
                          {caseItem.notes || "Successful payout received."}
                        </p>
                        <div className="flex items-center justify-between">
                          ...
                          <Link href={`/cases/${caseItem.id}`}>
                            <Button
                              size="sm"
                              variant="outline"
                              className="border-[#22C55E]/30 text-[#22C55E] hover:bg-[#22C55E]/10 opacity-0 group-hover:opacity-100 transition-all bg-transparent"
                            >
          ...
          {!isLoading && filteredCases.length === 0 && !error && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-[#E6E7EB] mb-2">No approvals found</h3>
