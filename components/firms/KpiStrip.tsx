...
export function KpiStrip({ agg }: KpiStripProps) {
  ...
  const denialsToday = useAnimatedCounter(agg?.denials_30d ? Math.floor(agg.denials_30d / 30) : 0, 800)
  const denials7d = useAnimatedCounter(agg?.denials_7d || 0, 1000)
  const denials30d = useAnimatedCounter(agg?.denials_30d || 0, 1200)
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <Card className="bg-white/80 backdrop-blur-sm border border-gray-200/50">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-red-500">{denialsToday}</div>
            <div className="text-sm text-gray-500">Denials Today</div>
          </CardContent>
        </Card>
        <Card className="bg-white/80 backdrop-blur-sm border border-gray-200/50">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-red-500">{denials7d}</div>
            <div className="text-sm text-gray-500">Denials 7d</div>
          </CardContent>
        </Card>
        <Card className="bg-white/80 backdrop-blur-sm border border-gray-200/50">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-red-500">{denials30d}</div>
